...
parser.add_argument(
    "--adjdata", type=str, default="store/adj_mx.pkl", help="adj data path"
)
...
parser.add_argument("--save", type=str, default="store/checkpoint", help="save path")
parser.add_argument(
    "--extensions_enabled",
    action="store_true",
    help="Graph Wavenet extensions for performance",
)
...
def main():
    ...
    engine = Model(
        scaler, args.num_nodes, args.learning_rate, args.weight_decay, device, adj_mx
    )
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y) in enumerate(dataloader["train_loader"].get_iterator()):
            ...
            if iter % args.print_every == 0:
                log = (
                    "Iter: {:03d}, Train Loss: {:.4f}, Train MAPE: "
                    + "{:.4f}, Train RMSE: {:.4f}"
                )
                print(
        ...
        if np.argmin(his_loss) == len(his_loss) - 1:
            torch.save(
                engine.gwnet.state_dict(), args.save + "/epoch_" + str(i) + ".pth"
            )
            best_epoch = i

        log = (
            "Epoch: {:03d}, Train Loss: {:.4f}, Train MAPE: {:.4f}, "
    ...
    for i in range(12):
        ...
        log = (
            "Evaluate best model on test data for horizon {:d}, Test MAE"
    ...
    log = (
        "On average over 12 horizons, Test MAE: {:.4f}, Test MAPE: "
        + "{:.4f}, Test RMSE: {:.4f}"
    )
